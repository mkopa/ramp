...
// Copyright (c) 2026 Marcin Kopa <marcin@marcin.info>.
...
// RAMP doesn't want to be updated every time those fields change, therefore always fill
    ...
    if !cues.is_empty() {
        ...
        for (idx, cue) in cues.iter().enumerate() {
            ...
            if !cue.tags.is_empty() {
                ...
                for (tag_idx, tag) in cue.tags.iter().enumerate() {
            ...
            if !cue.points.is_empty() {
                ...
                for (cue_point_idx, pt) in cue.points.iter().enumerate() {
                    println!(
                        "|                      [{:0>2}] Offset:    {:?}",
                        cue_point_idx + 1,
                        pt.start_offset_ts
...
fn print_visuals(visuals: &[Visual]) {
    if !visuals.is_empty() {
        ...
        for (idx, visual) in visuals.iter().enumerate() {
            ...
            for (tag_idx, tag) in visual.tags.iter().enumerate() {
                if let Some(std_key) = tag.std_key {
                    println!(
                        "{}",
                        print_tag_item(tag_idx + 1, &format!("{:?}", std_key), &tag.value, 21)
...
fn print_progress(ts: u64, dur: Option<u64>, tb: Option<TimeBase>) {
    ...
    // stdout once and use write! rather than print!.
    ...
    // This extra space is a workaround for Console to correctly erase the previous line.
    write!(output, " ").unwrap();
